...
namespace decx 
{
    namespace conv_I2R 
    {
        namespace GPUK {
            ...
            */
            void cu_conv_eq_mm_fp16(const float4* A, const float4* B, float4* dst, const ulong2 MatA_load_bounds,
                                    const uint dst_store_bound, const uint WB, const uint __iter);
            ...
            * @param __iter : how many iteration along B's width (128x)
            */
            void cu_conv_eq_mm_fp16_accu(const float4* A, const float4* B, float4* dst,
                                         const ulong2 MatA_load_bounds, const uint dst_store_bound, const uint WB, const uint __iter);
        }
    }
}

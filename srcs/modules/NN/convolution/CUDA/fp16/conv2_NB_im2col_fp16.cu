...
void decx::conv_I2R::conv2_NB_im2col_fp16(decx::_GPU_Tensor*         src,
                                      ...
                                      de::DH*                        handle)
{
    ...
    const uint4 dst_o_dim = make_uint4(src->get_layout().width - (kernel->get_layout().width / 2) * 2, 
                                       src->get_layout().height - (kernel->get_layout().height / 2) * 2,
                                       kernel->TensorNum(), 0);

    dst->re_construct(src->Type(), src->get_layout().width - (kernel->get_layout().width / 2) * 2,
        src->get_layout().height - (kernel->get_layout().height / 2) * 2, kernel->TensorNum());
    ...
    const ulong2 src_buf_dim = make_ulong2(src->get_layout().dp_x_wp, src->get_layout().height);
    ...
    for (int i = 0; i < kernel->TensorNum(); ++i) {
        checkCudaErrors(
            cudaMemcpy2DAsync(reinterpret_cast<de::Half*>(ker_buf.ptr) + i * ker_buf_dim.x,             kernel->get_layout().dpitch * kernel->get_layout().width * sizeof(de::Half),
...
template void decx::conv_I2R::conv2_NB_im2col_fp16<true>(decx::_GPU_Tensor* src, decx::_GPU_TensorArray* kernel,
    decx::_GPU_Tensor* dst, const int accu_flag, decx::cuda_stream* S, decx::cuda_event* E, de::DH* handle);

template void decx::conv_I2R::conv2_NB_im2col_fp16<false>(decx::_GPU_Tensor* src, decx::_GPU_TensorArray* kernel,
    decx::_GPU_Tensor* dst, const int accu_flag, decx::cuda_stream* S, decx::cuda_event* E, de::DH* handle);


template <bool _print>
void decx::conv_I2R::conv2_NB_im2col_fp16_stride(decx::_GPU_Tensor*         src,
                                             decx::_GPU_TensorArray*    kernel, 
                                             decx::_GPU_Tensor*         dst, 
                                             const int                  accu_flag, 
                                             decx::cuda_stream*         S,
...
{
    // the width and height of output tensor
    const uint4 dst_o_dim = make_uint4((src->get_layout().width - kernel->get_layout().width + 1) / strideXY.x, 
                                       (src->get_layout().height - kernel->get_layout().height + 1) / strideXY.y,
                                       kernel->TensorNum(), 0);
...
template void decx::conv_I2R::conv2_NB_im2col_fp16_stride<true>(decx::_GPU_Tensor* src, decx::_GPU_TensorArray* kernel,
    decx::_GPU_Tensor* dst, const int accu_flag, decx::cuda_stream* S, decx::cuda_event* E, const uint2 strideXY, de::DH* handle);

template void decx::conv_I2R::conv2_NB_im2col_fp16_stride<false>(decx::_GPU_Tensor* src, decx::_GPU_TensorArray* kernel,
    decx::_GPU_Tensor* dst, const int accu_flag, decx::cuda_stream* S, decx::cuda_event* E, const uint2 strideXY, de::DH* handle);
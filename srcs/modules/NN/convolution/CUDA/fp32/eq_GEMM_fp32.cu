...
__global__ void 
decx::conv_I2R::GPUK::cu_conv_eq_mm_fp32(const float4* __restrict       A,
                                         ...
                                         const uint                     __iter)                         // how many iteration along B's width (128x)
{
    ...
    float4 reg_A[8], reg_B[4][2], reg_sum[4];
    ...
    for (uint i = 0; i < __iter; ++i)
    {
        ...
        reg_B[0][0] = B[dex_B];
        reg_B[1][0] = B[dex_B + WB];
        reg_B[2][0] = B[dex_B + 2 * WB];
        reg_B[3][0] = B[dex_B + 3 * WB];
        
        frag_B[threadIdx.y * 4][threadIdx.x] = reg_B[0][0];
        frag_B[threadIdx.y * 4 + 1][threadIdx.x] = reg_B[1][0];
        frag_B[threadIdx.y * 4 + 2][threadIdx.x] = reg_B[2][0];
        frag_B[threadIdx.y * 4 + 3][threadIdx.x] = reg_B[3][0];
        ...
        for (uint load_A = 0; load_A < 4; ++load_A) {
            ...
            if (tidx < MatA_load_bounds.x && along_L_A < MatA_load_bounds.y) {
                reg_A[0] = A[dex_A];
                reg_A[1] = A[dex_A + MatA_load_bounds.x];
                reg_A[2] = A[dex_A + MatA_load_bounds.x * 2];
                reg_A[3] = A[dex_A + MatA_load_bounds.x * 3];
            }

            frag_A[threadIdx.y * 4][threadIdx.x] = reg_A[0];
            frag_A[threadIdx.y * 4 + 1][threadIdx.x] = reg_A[1];
            frag_A[threadIdx.y * 4 + 2][threadIdx.x] = reg_A[2];
            frag_A[threadIdx.y * 4 + 3][threadIdx.x] = reg_A[3];
            ...
            {
                reg_A[0] = frag_A[_L * 8][threadIdx.x];
                reg_A[1] = frag_A[_L * 8 + 1][threadIdx.x];
                reg_A[2] = frag_A[_L * 8 + 2][threadIdx.x];
                reg_A[3] = frag_A[_L * 8 + 3][threadIdx.x];
                reg_A[4] = frag_A[_L * 8 + 4][threadIdx.x];
                reg_A[5] = frag_A[_L * 8 + 5][threadIdx.x];
                reg_A[6] = frag_A[_L * 8 + 6][threadIdx.x];
                reg_A[7] = frag_A[_L * 8 + 7][threadIdx.x];

                reg_B[0][0] = frag_B[threadIdx.y * 4][load_A * 8 + _L * 2];
                reg_B[0][1] = frag_B[threadIdx.y * 4][load_A * 8 + _L * 2 + 1];
                reg_B[1][0] = frag_B[threadIdx.y * 4 + 1][load_A * 8 + _L * 2];
                reg_B[1][1] = frag_B[threadIdx.y * 4 + 1][load_A * 8 + _L * 2 + 1];
                reg_B[2][0] = frag_B[threadIdx.y * 4 + 2][load_A * 8 + _L * 2];
                reg_B[2][1] = frag_B[threadIdx.y * 4 + 2][load_A * 8 + _L * 2 + 1];
                reg_B[3][0] = frag_B[threadIdx.y * 4 + 3][load_A * 8 + _L * 2];
                reg_B[3][1] = frag_B[threadIdx.y * 4 + 3][load_A * 8 + _L * 2 + 1];

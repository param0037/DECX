...
void decx::conv_I2R::conv2_BC_im2col_fp32(decx::_GPU_Tensor*              src, 
                                      ...
                                      de::DH*                         handle)
{
    ...
    const uint4 dst_o_dim = make_uint4(src->get_layout().width, src->get_layout().height, kernel->TensorNum(), 0);
    ...
    const ulong2 src_buf_dim = make_ulong2((decx::utils::ceil<size_t>(src->get_layout().width + kernel->get_layout().width - 1, 4) * 4) * (size_t)src->get_layout().dpitch,
    ...
    for (int i = 0; i < kernel->TensorNum(); ++i) {
        cudaMemcpy2DAsync(reinterpret_cast<float*>(ker_buf.ptr) + i * ker_buf_dim.x,                kernel->get_layout().dpitch * kernel->get_layout().width * sizeof(float),
                          ...
                          kernel->get_layout().dpitch * kernel->get_layout().width * sizeof(float),                           kernel->get_layout().height,
                          cudaMemcpyDeviceToDevice,                                                 S->get_raw_stream_ref());
    }

    // copy data from src(host) to src_buffer(device)
    checkCudaErrors(cudaMemcpy2DAsync(
        reinterpret_cast<float*>(src_buf.ptr) + (kernel->get_layout().height / 2) * src_buf_dim.x + (kernel->get_layout().width / 2) * src->get_layout().dpitch,
        src_buf_dim.x * sizeof(float), 
        src->Tens.ptr,                                                              src->get_layout().dp_x_wp * sizeof(float),
        src->get_layout().dp_x_wp * sizeof(float),                                       src->get_layout().height,
        cudaMemcpyDeviceToDevice,                                                   S->get_raw_stream_ref()));
...
template void decx::conv_I2R::conv2_BC_im2col_fp32<true>(decx::_GPU_Tensor* src, decx::_GPU_TensorArray* kernel,
    decx::_GPU_Tensor* dst, decx::cuda_stream* S, decx::cuda_event* E, de::DH* handle);

template void decx::conv_I2R::conv2_BC_im2col_fp32<false>(decx::_GPU_Tensor* src, decx::_GPU_TensorArray* kernel,
    decx::_GPU_Tensor* dst, decx::cuda_stream* S, decx::cuda_event* E, de::DH* handle);
...
template <bool _print>
void decx::conv_I2R::conv2_BC_im2col_fp32_stride(decx::_GPU_Tensor*                 src, 
                                             decx::_GPU_TensorArray*            kernel, 
                                             decx::_GPU_Tensor*                 dst, 
                                             decx::cuda_stream*                 S, 
                                             decx::cuda_event*                  E,
...
{
    // the width and height of output tensor
    const uint4 dst_o_dim = make_uint4(src->get_layout().width / strideXY.x, src->get_layout().height / strideXY.y, kernel->TensorNum(), 0);
    // re-construct destinated matrix to match the shapes
...
template void decx::conv_I2R::conv2_BC_im2col_fp32_stride<true>(decx::_GPU_Tensor* src, decx::_GPU_TensorArray* kernel,
    decx::_GPU_Tensor* dst, decx::cuda_stream* S, decx::cuda_event* E, const uint2 strideXY, de::DH* handle);


template void decx::conv_I2R::conv2_BC_im2col_fp32_stride<false>(decx::_GPU_Tensor* src, decx::_GPU_TensorArray* kernel,
    decx::_GPU_Tensor* dst, decx::cuda_stream* S, decx::cuda_event* E, const uint2 strideXY, de::DH* handle);
                                 ...
                                 const uint                  _loop)
{
    ...
    if (f_mgr->frag_left_over != 0) {
        ...
        for (int i = 0; i < t1D->total_thread - 1; ++i) {
            t1D->_async_thread[i] = decx::cpu::register_task(&decx::thread_pool,decx::_conv2_rN_uint8_I32_ST,
        ...
        t1D->_async_thread[t1D->total_thread - 1] = decx::cpu::register_task(&decx::thread_pool,decx::_conv2_rN_uint8_I32_ST,
            tmp_src_ptr, kernel, tmp_dst_ptr,
                                 ...
                                 const uint                  _loop)
{
    ...
    if (f_mgr->frag_left_over != 0) {
        ...
        for (int i = 0; i < t1D->total_thread - 1; ++i) {
            t1D->_async_thread[i] = decx::cpu::register_task(&decx::thread_pool,decx::_conv2_rN_uint8_F32_ST,
                tmp_src_ptr, kernel, tmp_dst_ptr,
                make_uint2(proc_dim.x, f_mgr->frag_len), ker_dims, Wsrc, Wdst, reg_WL, _loop);
        ...
        t1D->_async_thread[t1D->total_thread - 1] = decx::cpu::register_task(&decx::thread_pool,decx::_conv2_rN_uint8_F32_ST,
            tmp_src_ptr, kernel, tmp_dst_ptr,
            make_uint2(proc_dim.x, f_mgr->frag_left_over), ker_dims, Wsrc, Wdst, reg_WL, _loop);
    }
    else {
                                 ...
                                 const uint                  _loop)
{
    ...
    if (f_mgr->frag_left_over != 0) {
        ...
        for (int i = 0; i < t1D->total_thread - 1; ++i) {
            t1D->_async_thread[i] = decx::cpu::register_task(&decx::thread_pool,decx::_conv2_rN_uint8_uint8_ST,
                tmp_src_ptr, kernel, tmp_dst_ptr,
                make_uint2(proc_dim.x, f_mgr->frag_len), ker_dims, Wsrc, Wdst, reg_WL, _loop);
        ...
        t1D->_async_thread[t1D->total_thread - 1] = decx::cpu::register_task(&decx::thread_pool,decx::_conv2_rN_uint8_uint8_ST,
            tmp_src_ptr, kernel, tmp_dst_ptr,
            make_uint2(proc_dim.x, f_mgr->frag_left_over), ker_dims, Wsrc, Wdst, reg_WL, _loop);
    }
    else {

...
_THREAD_FUNCTION_
void decx::_conv2_rN_fp32_ST(float* __restrict     src,
...
_THREAD_FUNCTION_
void decx::_conv2_r1_r4_fp32_ST(float* __restrict   src, 
                                float* __restrict   kernel, 
                                float* __restrict   dst, 
                                const uint2         proc_dim, 
                                const uint2         ker_dims,
...
void decx::conv::_conv2_rN_fp32_caller(float*                src,
                                 float*                     kernel, 
                                 float*                      dst, 
                                 const uint2                 proc_dim, 
                                 const uint2                 ker_dims,
                                 ...
                                 const uint                  _loop)
{
    ...
    if (f_mgr->frag_left_over != 0) {
        ...
        for (int i = 0; i < t1D->total_thread - 1; ++i) {
            t1D->_async_thread[i] = decx::cpu::register_task(&decx::thread_pool,decx::_conv2_rN_fp32_ST,
        ...
        t1D->_async_thread[t1D->total_thread - 1] = decx::cpu::register_task(&decx::thread_pool,decx::_conv2_rN_fp32_ST,
            tmp_src_ptr, kernel, tmp_dst_ptr,
...
void decx::conv::_conv2_r1_r4_fp32_caller(float*                src,
                                    float*                kernel, 
                                    float*                      dst, 
                                    const uint2                 proc_dim, 
                                    const uint2                 ker_dims,
                                    ...
                                    decx::utils::frag_manager*  f_mgr)
{
    ...
    if (f_mgr->frag_left_over != 0) {
        ...
        for (int i = 0; i < t1D->total_thread - 1; ++i) {
            t1D->_async_thread[i] = decx::cpu::register_task(&decx::thread_pool,decx::_conv2_r1_r4_fp32_ST,
                tmp_src_ptr, kernel, tmp_dst_ptr,
        ...
        t1D->_async_thread[t1D->total_thread - 1] = decx::cpu::register_task(&decx::thread_pool,decx::_conv2_r1_r4_fp32_ST,
            tmp_src_ptr, kernel, tmp_dst_ptr,

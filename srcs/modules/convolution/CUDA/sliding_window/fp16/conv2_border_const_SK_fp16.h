...
#ifndef _CONV2_BORDER_CONST_SK_FP16_H_
#define _CONV2_BORDER_CONST_SK_FP16_H_
    ...
    decx::cuda_stream* S[3], const int flag)
{
    ...
    main_loop_regulable_R_sk(
        hconv2_kernel_within8x8(Dmem1->mem, Dmem3->mem, src_diff, *Dsrc_alloc_dim, *Ddst_alloc_dim, *ker_dim, S[0], flag),
        hconv2_kernel_within8x8(Dmem2->mem, Dmem4->mem, src_diff, *Dsrc_alloc_dim, *Ddst_alloc_dim, *ker_dim, S[0], flag),
...
namespace decx
{
    static void _Conv2_BC_R8x8_SK_fp16(
        decx::_MatrixArray* src, decx::_Matrix* kernel, decx::_MatrixArray* dst, de::DH *handle, const int flag);


    static void _Conv2_BC_R8x16_SK_fp16(
        decx::_MatrixArray* src, decx::_Matrix* kernel, decx::_MatrixArray* dst, de::DH *handle, const int flag);


    static void _Conv2_BC_R16x8_SK_fp16(
        decx::_MatrixArray* src, decx::_Matrix* kernel, decx::_MatrixArray* dst, de::DH *handle, const int flag);


    static void _Conv2_BC_R16x16_SK_fp16(
        decx::_MatrixArray* src, decx::_Matrix* kernel, decx::_MatrixArray* dst, de::DH *handle, const int flag);


    static void hConv2_border_zero_sk(
        decx::_MatrixArray* src, decx::_Matrix* kernel, decx::_MatrixArray* dst, de::DH* handle, const int flag);
}


// single kernel
static void decx::_Conv2_BC_R8x8_SK_fp16(
    decx::_MatrixArray* src, decx::_Matrix* kernel, decx::_MatrixArray* dst, de::DH *handle, const int flag)
{
    ...
    for (int k = 0; k < kernel->height; ++k) {
        cudaMemcpyToSymbolAsync(Const_Mem, (de::Half*)kernel->Mat.ptr + offset_ker,
    ...
    decx::alloc::_device_dealloc(&r_Dmem1);             decx::alloc::_device_dealloc(&r_Dmem2);
    decx::alloc::_device_dealloc(&r_Dmem3);             decx::alloc::_device_dealloc(&r_Dmem4);
...
// single kernel
static void decx::_Conv2_BC_R8x16_SK_fp16(
    decx::_MatrixArray* src, decx::_Matrix* kernel, decx::_MatrixArray* dst, de::DH* handle, const int flag)
{
    MIF<float4> Dmem1, Dmem2,    // for src
        Dmem3, Dmem4;            // for dst
...
// single kernel
static void decx::_Conv2_BC_R16x8_SK_fp16(
    decx::_MatrixArray* src, decx::_Matrix* kernel, decx::_MatrixArray* dst, de::DH* handle, const int flag)
{
    MIF<float4> Dmem1, Dmem2,    // for src
        Dmem3, Dmem4;            // for dst
...
// single kernel
static void decx::_Conv2_BC_R16x16_SK_fp16(
    decx::_MatrixArray* src, decx::_Matrix* kernel, decx::_MatrixArray* dst, de::DH* handle, const int flag)
{
    MIF<float4> Dmem1, Dmem2,    // for src
        Dmem3, Dmem4;            // for dst
...
static void decx::hConv2_border_zero_sk(
    decx::_MatrixArray* src, decx::_Matrix* kernel, decx::_MatrixArray* dst, de::DH* handle, const int flag)
{
    int2 half_ker_dim;
    ...
    if (half_ker_dim.x < bounded_kernel_R8 + 1) {
        if (half_ker_dim.y < bounded_kernel_R8 + 1) {
            decx::_Conv2_BC_R8x8_SK_fp16(src, kernel, dst, handle, flag);
        }
        else {
            decx::_Conv2_BC_R16x8_SK_fp16(src, kernel, dst, handle, flag);
        }
    }
    else {
        if (half_ker_dim.y < bounded_kernel_R8 + 1) {
            decx::_Conv2_BC_R8x16_SK_fp16(src, kernel, dst, handle, flag);
        }
        else {
            decx::_Conv2_BC_R16x16_SK_fp16(src, kernel, dst, handle, flag);
        }
    }
    decx::Success(handle);
}

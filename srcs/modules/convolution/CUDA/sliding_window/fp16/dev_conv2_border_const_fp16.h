...
namespace decx
{
    ...
    */
    static void _dev_Conv2_BC_R8x8_fp16(decx::_GPU_Matrix* src, decx::_GPU_Matrix* kernel, decx::_GPU_Matrix* dst, 
    ...
    * In order to save the device memory, just allocate memory of suitable size
    */
    static void _dev_Conv2_BC_R8x16_fp16(decx::_GPU_Matrix* src, decx::_GPU_Matrix* kernel, decx::_GPU_Matrix* dst, 
        de::DH* handle, const int flag);
    ...
    * In order to save the device memory, just allocate memory of suitable size
    */
    static void _dev_Conv2_BC_R16x8_fp16(decx::_GPU_Matrix* src, decx::_GPU_Matrix* kernel, decx::_GPU_Matrix* dst, 
        de::DH* handle, const int flag);
    ...
    * In order to save the device memory, just allocate memory of suitable size
    */
    static void _dev_Conv2_BC_R16x16_fp16(decx::_GPU_Matrix* src, decx::_GPU_Matrix* kernel, decx::_GPU_Matrix* dst, 
        de::DH* handle, const int flag);



    static void dev_hConv2_border_zero(decx::_GPU_Matrix& src, decx::_GPU_Matrix& kernel, decx::_GPU_Matrix& dst, 
        de::DH* handle, const int flag);
}
...
static
void decx::_dev_Conv2_BC_R8x8_fp16(decx::_GPU_Matrix* src, decx::_GPU_Matrix* kernel, decx::_GPU_Matrix* dst, 
    de::DH *handle, const int flag)
{
    float4* Dsrc,
        *Ddst;
    ...
    if (ker_dim.x == (bounded_kernel_R8 * 2 + 1) && ker_dim.y == (bounded_kernel_R8 * 2 + 1)) {
        decx::hconv2_kernel_exact8x8(Dsrc, Ddst, Dsrc_alloc_dim, Ddst_alloc_dim, ker_dim, S, flag);
    }
    else {
        int2 src_diff;
        src_diff.x = bounded_kernel_R8 - ker_dim.y / 2;                src_diff.y = bounded_kernel_R8 - ker_dim.x / 2;
        
        decx::hconv2_kernel_within8x8(Dsrc, Ddst, src_diff, Dsrc_alloc_dim, Ddst_alloc_dim, ker_dim, S, flag);
    }
...
static
void decx::_dev_Conv2_BC_R8x16_fp16(decx::_GPU_Matrix* src, decx::_GPU_Matrix* kernel, decx::_GPU_Matrix* dst, 
    de::DH *handle, const int flag)
{
    float4* Dsrc,
        *Ddst;
    ...
    if (ker_dim.x == (bounded_kernel_R16 * 2 + 1) && ker_dim.y == (bounded_kernel_R8 * 2 + 1)) {
        decx::hconv2_kernel_exact8x16(Dsrc, Ddst, Dsrc_alloc_dim, Ddst_alloc_dim, ker_dim, S, flag);
    }
    else {
        int2 src_diff;
        src_diff.x = bounded_kernel_R8 - ker_dim.y / 2;                src_diff.y = bounded_kernel_R16 - ker_dim.x / 2;
        
        decx::hconv2_kernel_within8x16(Dsrc, Ddst, src_diff, Dsrc_alloc_dim, Ddst_alloc_dim, ker_dim, S, flag);
    }
...
static
void decx::_dev_Conv2_BC_R16x8_fp16(decx::_GPU_Matrix* src, decx::_GPU_Matrix* kernel, decx::_GPU_Matrix* dst, 
    de::DH *handle, const int flag)
{
    float4* Dsrc,
        *Ddst;
    ...
    if (ker_dim.x == (bounded_kernel_R8 * 2 + 1) && ker_dim.y == (bounded_kernel_R16 * 2 + 1)) {
        decx::hconv2_kernel_exact16x8(Dsrc, Ddst, Dsrc_alloc_dim, Ddst_alloc_dim, ker_dim, S, flag);
    }
    else {
        int2 src_diff;
        src_diff.x = bounded_kernel_R16 - ker_dim.y / 2;                src_diff.y = bounded_kernel_R8 - ker_dim.x / 2;

        decx::hconv2_kernel_within16x8(Dsrc, Ddst, src_diff, Dsrc_alloc_dim, Ddst_alloc_dim, ker_dim, S, flag);
    }
...
static
void decx::_dev_Conv2_BC_R16x16_fp16(decx::_GPU_Matrix* src, decx::_GPU_Matrix* kernel, decx::_GPU_Matrix* dst, 
    de::DH *handle, const int flag)
{
    float4* Dsrc,
        * Ddst;
    ...
    if (ker_dim.x == (bounded_kernel_R16 * 2 + 1) && ker_dim.y == (bounded_kernel_R16 * 2 + 1)) {    
        decx::hconv2_kernel_exact16x16(Dsrc, Ddst, Dsrc_alloc_dim, Ddst_alloc_dim, ker_dim, S, flag);
    }
    else {
        int2 src_diff;
        src_diff.x = bounded_kernel_R16 - ker_dim.y / 2;                src_diff.y = bounded_kernel_R16 - ker_dim.x / 2;
        
        decx::hconv2_kernel_within16x16(Dsrc, Ddst, src_diff, Dsrc_alloc_dim, Ddst_alloc_dim, ker_dim, S, flag);
    }
...
static 
void decx::dev_hConv2_border_zero(decx::_GPU_Matrix& src, decx::_GPU_Matrix& kernel, decx::_GPU_Matrix& dst, 
    de::DH* handle, const int flag)
{
    int2 half_ker_dim;
    ...
    if (half_ker_dim.x < bounded_kernel_R8 + 1) {
        if (half_ker_dim.y < bounded_kernel_R8 + 1) {
            decx::_dev_Conv2_BC_R8x8_fp16(&src, &kernel, &dst, handle, flag);
        }
        else {
            decx::_dev_Conv2_BC_R16x8_fp16(&src, &kernel, &dst, handle, flag);
        }
    }
    else {
        if (half_ker_dim.y < bounded_kernel_R8 + 1) {
            decx::_dev_Conv2_BC_R8x16_fp16(&src, &kernel, &dst, handle, flag);
        }
        else {
            decx::_dev_Conv2_BC_R16x16_fp16(&src, &kernel, &dst, handle, flag);
        }
    }
}

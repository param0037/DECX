...
#ifndef _DEV_GPU_FFT1D_SUB_FUNCTION_H_
#define _DEV_GPU_FFT1D_SUB_FUNCTION_H_
...
namespace decx
{
    namespace signal
    {
        static void dev_GPU_FFT1D_R2C_fp32_organizer(decx::_GPU_Vector* src, decx::_GPU_Vector* dst, 
            decx::signal::CUDA_FFT_Configs* _conf, de::DH* handle, decx::cuda_stream* S, decx::cuda_event* E);


        static void dev_GPU_FFT1D_C2C_fp32_organizer(decx::_GPU_Vector* src, decx::_GPU_Vector* dst,
            decx::signal::CUDA_FFT_Configs* _conf, de::DH* handle, decx::cuda_stream* S, decx::cuda_event* E);
    }
}


static void decx::signal::dev_GPU_FFT1D_R2C_fp32_organizer(decx::_GPU_Vector* src, decx::_GPU_Vector* dst,
    decx::signal::CUDA_FFT_Configs* _conf, de::DH* handle, decx::cuda_stream* S, decx::cuda_event* E)
{
    ...
    for (int i = 0; i < _conf->_base_num; ++i) {
        ...
        case 2:
            if (count == 0) {
                decx::signal::CUDA_FFT1D_R2_R2C_first_caller((de::CPf*)src->Vec.ptr, MIF_1.mem, src->length, S);
        ...
        case 3:
            if (count == 0) {
                decx::signal::CUDA_FFT1D_R3_R2C_first_caller((de::CPf*)src->Vec.ptr, MIF_1.mem, src->length, S);
                decx::utils::set_mutex_memory_state<de::CPf, de::CPf>(&MIF_1, &MIF_0);
            }
            else if (count == _conf->_base_num - 1) {
        ...
        case 4:
            if (count == 0) {
                decx::signal::CUDA_FFT1D_R4_R2C_first_caller((float*)src->Vec.ptr, MIF_1.mem, src->length, S);
                decx::utils::set_mutex_memory_state<de::CPf, de::CPf>(&MIF_1, &MIF_0);
            }
            else if (count == _conf->_base_num - 1) {
        ...
        case 5:
            if (count == 0) {
                decx::signal::CUDA_FFT1D_R5_R2C_first_caller((float*)src->Vec.ptr, MIF_1.mem, src->length, S);
                decx::utils::set_mutex_memory_state<de::CPf, de::CPf>(&MIF_1, &MIF_0);
            }
            else if (count == _conf->_base_num - 1) {
...
static void decx::signal::dev_GPU_FFT1D_C2C_fp32_organizer(decx::_GPU_Vector* src, decx::_GPU_Vector* dst,
    decx::signal::CUDA_FFT_Configs* _conf, de::DH* handle, decx::cuda_stream* S, decx::cuda_event* E)
{
    decx::PtrInfo<de::CPf> dev_tmp0, dev_tmp1;
    if (decx::alloc::_device_malloc(&dev_tmp0, dst->_length * sizeof(de::CPf), true, S)) {
    ...
    for (int i = 0; i < _conf->_base_num; ++i) {
        ...
        case 2:
            if (count == 0) {
                decx::signal::CUDA_FFT1D_R2_C2C_first_caller<false>((de::CPf*)src->Vec.ptr, MIF_1.mem, src->length, S);
                decx::utils::set_mutex_memory_state<de::CPf, de::CPf>(&MIF_1, &MIF_0);
            }
            else if (count == _conf->_base_num - 1) {
                decx::signal::dev_CUDA_FFT1D_R2_C2C_last_caller(&MIF_0, &MIF_1, (de::CPf*)dst->Vec.ptr, src->length, warp_proc_len, S);
        ...
        case 3:
            if (count == 0) {
                decx::signal::CUDA_FFT1D_R3_C2C_first_caller<false>((de::CPf*)src->Vec.ptr, MIF_1.mem, src->length, S);
                decx::utils::set_mutex_memory_state<de::CPf, de::CPf>(&MIF_1, &MIF_0);
            }
            else if (count == _conf->_base_num - 1) {
                decx::signal::dev_CUDA_FFT1D_R3_C2C_last_caller(&MIF_0, &MIF_1, (de::CPf*)dst->Vec.ptr, src->length, warp_proc_len, S);
        ...
        case 4:
            if (count == 0) {
                decx::signal::CUDA_FFT1D_R4_C2C_first_caller<false>((de::CPf*)src->Vec.ptr, MIF_1.mem, src->length, S);
                decx::utils::set_mutex_memory_state<de::CPf, de::CPf>(&MIF_1, &MIF_0);
            }
            else if (count == _conf->_base_num - 1) {
                decx::signal::dev_CUDA_FFT1D_R4_C2C_last_caller(&MIF_0, &MIF_1, (de::CPf*)dst->Vec.ptr, src->length, warp_proc_len, S);

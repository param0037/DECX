...
#ifndef _DEV_GPU_IFFT2D_SUB_FUNCTION_H_
#define _DEV_GPU_IFFT2D_SUB_FUNCTION_H_
...
namespace decx
{
    namespace signal
    {
        static void dev_GPU_IFFT2D_C2C_fp32_organizer(decx::_GPU_Matrix* src, decx::_GPU_Matrix* dst,
            decx::signal::CUDA_FFT_Configs _conf[2], de::DH* handle, decx::cuda_stream* S);


        static void dev_GPU_IFFT2D_C2R_fp32_organizer(decx::_GPU_Matrix* src, decx::_GPU_Matrix* dst,
            decx::signal::CUDA_FFT_Configs _conf[2], de::DH* handle, decx::cuda_stream* S);
    }
}
...
static void decx::signal::dev_GPU_IFFT2D_C2C_fp32_organizer(decx::_GPU_Matrix* src, decx::_GPU_Matrix* dst,
    decx::signal::CUDA_FFT_Configs _conf[2], de::DH* handle, decx::cuda_stream* S)
{
    ...
    decx::alloc::MIF<de::CPf> MIF_0(dev_tmp0.ptr, false),
    ...
    for (int i = 0; i < _conf[0]._base_num; ++i) {
        ...
        case 2:
            if (i == 0) {
                decx::signal::gpu::dev_CUDA_FFT2D_R2_C2C_first_caller<true>((de::CPf*)src->Mat.ptr, &MIF_0, src->Width(),
        ...
        case 3:
            if (i == 0) {
                decx::signal::gpu::dev_CUDA_FFT2D_R3_C2C_first_caller<true>((de::CPf*)src->Mat.ptr, &MIF_0, src->Width(),
                    tmp_dims.x, src->Height(), S);
            }
            else {
        ...
        case 4:
            if (i == 0) {
                decx::signal::gpu::dev_CUDA_FFT2D_R4_C2C_first_caller<true>((de::CPf*)src->Mat.ptr, &MIF_0, src->Width(),
                    tmp_dims.x, src->Height(), S);
            }
            else {
        ...
        case 5:
            if (i == 0) {
                decx::signal::gpu::dev_CUDA_FFT2D_R5_C2C_first_caller<true>((de::CPf*)src->Mat.ptr, &MIF_0, src->Width(),
                    tmp_dims.x, src->Height(), S);
            }
            else {
    ...
    if (MIF_0.leading) {
        ...
        decx::signal::utils::gpu::_dev_FFT2D_transpose_C2C_div_Async(&MIF_0, (de::CPf*)dst->Mat.ptr,
            tmp_dims.y / 2, tmp_dims.x / 2, make_uint2(tmp_dims.x, tmp_dims.y), src->Height(), S);
    }
    else {
        // copy back the data from device to host
        decx::signal::utils::gpu::_dev_FFT2D_transpose_C2C_div_Async(&MIF_1, (de::CPf*)dst->Mat.ptr,
            tmp_dims.y / 2, tmp_dims.x / 2, make_uint2(tmp_dims.x, tmp_dims.y), src->Height(), S);
    }
...
static void decx::signal::dev_GPU_IFFT2D_C2R_fp32_organizer(decx::_GPU_Matrix* src, decx::_GPU_Matrix* dst,
    decx::signal::CUDA_FFT_Configs _conf[2], de::DH* handle, decx::cuda_stream* S)
{
    const uint2 tmp_dims = make_uint2(dst->Pitch(),       // aligned to 4 already (4 de::CPf) -> (2 double2)
        decx::utils::ceil<uint>(src->Height(), 4) * 4);
    ...
    for (int i = 0; i < _conf[0]._base_num;) {
        ...
        case 2:
            if (i == 0) {
                decx::signal::gpu::dev_CUDA_FFT2D_R2_C2C_first_caller<true>((de::CPf*)src->Mat.ptr, &MIF_1, src->Width(),
                    tmp_dims.x, src->Height(), S);
            }
            else {
                decx::signal::gpu::CUDA_FFT2D_R2_C2C_caller(&MIF_0, &MIF_1, src->Width(), tmp_dims.x,
        ...
        case 3:
            if (i == 0) {
                decx::signal::gpu::dev_CUDA_FFT2D_R3_C2C_first_caller<true>((de::CPf*)src->Mat.ptr, &MIF_1, src->Width(),
                    tmp_dims.x, src->Height(), S);
            }
            else {
                decx::signal::gpu::CUDA_FFT2D_R3_C2C_caller(&MIF_0, &MIF_1, src->Width(), tmp_dims.x,
        ...
        case 4:
            if (i == 0) {
                decx::signal::gpu::dev_CUDA_FFT2D_R4_C2C_first_caller<true>((de::CPf*)src->Mat.ptr, &MIF_1, src->Width(),
                    tmp_dims.x, src->Height(), S);
            }
            else {
                decx::signal::gpu::CUDA_FFT2D_R4_C2C_caller(&MIF_0, &MIF_1, src->Width(), tmp_dims.x,
        ...
        case 5:
            if (i == 0) {
                decx::signal::gpu::dev_CUDA_FFT2D_R5_C2C_first_caller<true>((de::CPf*)src->Mat.ptr, &MIF_1, src->Width(),
                    tmp_dims.x, src->Height(), S);
            }
            else {
                decx::signal::gpu::CUDA_FFT2D_R5_C2C_caller(&MIF_0, &MIF_1, src->Width(), tmp_dims.x,
    ...
    if (MIF_0.leading) {
        // copy back the data from device to host
        decx::signal::utils::gpu::_dev_IFFT2D_transpose_R2R_Async(&MIF_0, (de::CPf*)dst->Mat.ptr,
            tmp_dims.y / 2, dst->Pitch() / 4, make_uint2(tmp_dims.x, tmp_dims.y), S);
    }
    else {
        // copy back the data from device to host
        decx::signal::utils::gpu::_dev_IFFT2D_transpose_R2R_Async(&MIF_1, (de::CPf*)dst->Mat.ptr,
            tmp_dims.y / 2, dst->Pitch() / 4, make_uint2(tmp_dims.x, tmp_dims.y), S);
    }

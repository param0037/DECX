...
#ifndef _DEV_GPU_FFT2D_SUB_FUNCTION_H_
#define _DEV_GPU_FFT2D_SUB_FUNCTION_H_
...
namespace decx
{
    namespace signal
    {
        static void dev_GPU_FFT2D_R2C_fp32_organizer(decx::_GPU_Matrix* src, decx::_GPU_Matrix* dst,
            decx::signal::CUDA_FFT_Configs _conf[2], de::DH* handle, decx::cuda_stream* S, decx::cuda_event* E);


        static void dev_GPU_FFT2D_C2C_fp32_organizer(decx::_GPU_Matrix* src, decx::_GPU_Matrix* dst,
            decx::signal::CUDA_FFT_Configs _conf[2], de::DH* handle, decx::cuda_stream* S, decx::cuda_event* E);
    }
}
...
static void decx::signal::dev_GPU_FFT2D_R2C_fp32_organizer(decx::_GPU_Matrix* src, decx::_GPU_Matrix* dst,
    decx::signal::CUDA_FFT_Configs _conf[2], de::DH* handle, decx::cuda_stream* S, decx::cuda_event* E)
{
    ...
    for (int i = 0; i < _conf[0]._base_num; ++i) {
        ...
        case 2:
            if (i == 0) {
                decx::signal::gpu::dev_CUDA_FFT2D_R2_R2C_first_caller((float*)src->Mat.ptr, &MIF_0, src->Width(),
                    src->Pitch(), tmp_dims.x, src->Height(), S); }
            else {
        ...
        case 3:
            if (i == 0) {
                decx::signal::gpu::dev_CUDA_FFT2D_R3_R2C_first_caller((float*)src->Mat.ptr, &MIF_0, src->Width(),
                    src->Pitch(), tmp_dims.x, src->Height(), S); }
            else {
        ...
        case 4:
            if (i == 0) {
                decx::signal::gpu::dev_CUDA_FFT2D_R4_R2C_first_caller((float*)src->Mat.ptr, &MIF_0, src->Width(), 
                    src->Pitch(), tmp_dims.x, src->Height(), S); }
            else {
        ...
        case 5:
            if (i == 0) {
                decx::signal::gpu::dev_CUDA_FFT2D_R5_R2C_first_caller((float*)src->Mat.ptr, &MIF_0, src->Width(),
                    src->Pitch(), tmp_dims.x, src->Height(), S); }
            else {
    ...
    if (MIF_0.leading) {
        ...
        decx::signal::utils::gpu::_dev_FFT2D_transpose_C2C_Async(&MIF_0, (de::CPf*)dst->Mat.ptr,
            tmp_dims.y / 2, dst->Pitch() / 2, make_uint2(dst->Width(), dst->Height()), S);
    }
    else {
        // copy back the data from device to host
        decx::signal::utils::gpu::_dev_FFT2D_transpose_C2C_Async(&MIF_1, (de::CPf*)dst->Mat.ptr,
            tmp_dims.y / 2, dst->Pitch() / 2, make_uint2(dst->Width(), dst->Height()), S);
    }
...
static void decx::signal::dev_GPU_FFT2D_C2C_fp32_organizer(decx::_GPU_Matrix* src, decx::_GPU_Matrix* dst,
    decx::signal::CUDA_FFT_Configs _conf[2], de::DH* handle, decx::cuda_stream* S, decx::cuda_event* E)
{
    const uint2 tmp_dims = make_uint2(dst->Pitch(),       // aligned to 4 already (4 de::CPf) -> (2 double2), which is equal to src->Pitch()
        decx::utils::ceil<uint>(src->Height(), 4) * 4);
    ...
    for (int i = 0; i < _conf[0]._base_num; ++i) {
        ...
        case 2:
            if (i == 0) {
                decx::signal::gpu::dev_CUDA_FFT2D_R2_C2C_first_caller<false>((de::CPf*)src->Mat.ptr, &MIF_0, src->Width(),
        ...
        case 3:
            if (i == 0) {
                decx::signal::gpu::dev_CUDA_FFT2D_R3_C2C_first_caller<false>((de::CPf*)src->Mat.ptr, &MIF_0, src->Width(),
                    tmp_dims.x, src->Height(), S);
            }
            else {
                decx::signal::gpu::CUDA_FFT2D_R3_C2C_caller(&MIF_0, &MIF_1, src->Width(), tmp_dims.x,
        ...
        case 4:
            if (i == 0) {
                decx::signal::gpu::dev_CUDA_FFT2D_R4_C2C_first_caller<false>((de::CPf*)src->Mat.ptr, &MIF_0, src->Width(),
                    tmp_dims.x, src->Height(), S);
            }
            else {
                decx::signal::gpu::CUDA_FFT2D_R4_C2C_caller(&MIF_0, &MIF_1, src->Width(), tmp_dims.x,
        ...
        case 5:
            if (i == 0) {
                decx::signal::gpu::dev_CUDA_FFT2D_R5_C2C_first_caller<false>((de::CPf*)src->Mat.ptr, &MIF_0, src->Width(),
                    tmp_dims.x, src->Height(), S);
            }
            else {
                decx::signal::gpu::CUDA_FFT2D_R5_C2C_caller(&MIF_0, &MIF_1, src->Width(), tmp_dims.x,

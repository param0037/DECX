    ...
    const float* __restrict src, double* __restrict dst, const size_t signal_length)
{
    ...
    if (f_mgr->is_left) {
        for (int i = 0; i < t1D->total_thread - 1; ++i) {
            ...
            t1D->_async_thread[i] = decx::cpu::register_task_default( _kernel_ptr, src, dst, signal_length, start_end);
            start_end.x += f_mgr->frag_len;
        }
        start_end.y += f_mgr->frag_left_over;
        t1D->_async_thread[t1D->total_thread - 1] = decx::cpu::register_task_default( _kernel_ptr, src, dst, signal_length, start_end);
    }
    else {

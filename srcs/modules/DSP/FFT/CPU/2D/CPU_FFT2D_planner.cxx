...
template <typename _type_in>
void decx::dsp::fft::cpu_FFT2D_planner<_type_in>::plan(decx::utils::_thread_arrange_1D* t1D, de::DH* handle)
{
    ...
    const uint32_t _conc_FFT_1D_H = min(decx::utils::ceil<uint32_t>(this->_signal_dims.y, 4), _concurrency);
    decx::utils::frag_manager_gen_Nx(&this->_thread_dist_FFTH, this->_signal_dims.y, _conc_FFT_1D_H, 4);

    // Thread distribution in FFT_W
    const uint32_t _conc_FFT_1D_V = min(decx::utils::ceil<uint32_t>(this->_signal_dims.x, 4), _concurrency);
...
template void decx::dsp::fft::cpu_FFT2D_planner<float>::plan(decx::utils::_thread_arrange_1D*, de::DH*);
...
template <typename _type_in>
const decx::dsp::fft::cpu_FFT1D_smaller<_type_in>* decx::dsp::fft::cpu_FFT2D_planner<_type_in>::get_FFTH_info() const
{
    return &this->_FFT_H;
}

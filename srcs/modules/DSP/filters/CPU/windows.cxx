...
de::signal::cpu::Gaussian_Window2D(de::Matrix& src, de::Matrix& dst, const de::Point2D_f u, const de::Point2D_f sigma, const float p)
{
    ...
    if (_src->Height() > decx::cpu::_get_permitted_concurrency()) {
        ...
        for (int i = 0; i < t1D.total_thread - 1; ++i) {
            if (_corrolated) {
                t1D._async_thread[i] = decx::cpu::register_task_default( decx::signal::CPUK::Gaussian_Window2D_cpl32,
                    _loc_src, _loc_dst,
                ...
                t1D._async_thread[i] = decx::cpu::register_task_default( decx::signal::CPUK::Gaussian_Window2D_cpl32_no_corrolation,
                    _loc_src, _loc_dst,
        ...
        if (_corrolated) {
            t1D._async_thread[t1D.total_thread - 1] = decx::cpu::register_task_default( decx::signal::CPUK::Gaussian_Window2D_cpl32,
                _loc_src, _loc_dst,
                make_float2(u.x, u.y), make_float2(sigma.x, sigma.y), p,
                _proc_dims, real_bound,
            ...
            t1D._async_thread[t1D.total_thread - 1] = decx::cpu::register_task_default( decx::signal::CPUK::Gaussian_Window2D_cpl32_no_corrolation,
                _loc_src, _loc_dst,
                make_float2(u.x, u.y), make_float2(sigma.x, sigma.y),
                _proc_dims, real_bound,
                f_mgr.frag_len * (t1D.total_thread - 1), pitch);
...
de::signal::cpu::Cone_Window2D(de::Matrix& src, de::Matrix& dst, const de::Point2D origin, const float radius)
{
    ...
    if (_src->Height() > decx::cpu::_get_permitted_concurrency()) {
        ...
        for (int i = 0; i < t1D.total_thread - 1; ++i) {
            t1D._async_thread[i] = decx::cpu::register_task_default( decx::signal::CPUK::Cone_Window2D_cpl32,
        ...
        _proc_dims.y = f_mgr.is_left ? f_mgr.frag_left_over : f_mgr.frag_len;
        t1D._async_thread[t1D.total_thread - 1] = decx::cpu::register_task_default( decx::signal::CPUK::Cone_Window2D_cpl32,
            _loc_src, _loc_dst, make_uint2(origin.x, origin.y), radius,
            _proc_dims, real_bound,
            f_mgr.frag_len * (t1D.total_thread - 1), pitch);

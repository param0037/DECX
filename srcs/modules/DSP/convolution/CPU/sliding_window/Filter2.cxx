...
namespace decx
{
    namespace conv 
    {
        ...
        template <bool _print>
        void Filter2D_uint8(decx::_Matrix* src, decx::_Matrix* kernel, decx::_Matrix* dst, const int flag, de::DH* handle, const int _output_type);
    }
}
...
void decx::conv::Filter2D_fp32(decx::_Matrix* src, decx::_Matrix* kernel, decx::_Matrix* dst, const int flag, de::DH* handle)
{
    ...
    case decx::bp::extend_label::_EXTEND_NONE_:
        dst->re_construct(src->Type(), src->Width() - kernel->Width() + 1, src->Height() - kernel->Height() + 1, src->get_store_type());
        decx::conv::_conv2_fp32_NB<_print>(src, kernel, dst, handle);
        break;

    case decx::bp::extend_label::_EXTEND_CONSTANT_:
        dst->re_construct(src->Type(), src->Width(), src->Height(), src->get_store_type());
...
template <bool _print>
void decx::conv::Filter2D_uint8(decx::_Matrix* src, decx::_Matrix* kernel, decx::_Matrix* dst, const int flag, de::DH* handle, const int _output_type)
{
    ...
    {
    case decx::bp::extend_label::_EXTEND_NONE_:
        dst->re_construct(_output_type, src->Width() - kernel->Width() + 1, src->Height() - kernel->Height() + 1, src->get_store_type());
        decx::conv::_conv2_uint8_NB<_print>(src, kernel, tmp_ker.ptr, dst, handle, _output_type);
        break;

    case decx::bp::extend_label::_EXTEND_CONSTANT_:
        dst->re_construct(_output_type, src->Width(), src->Height(), src->get_store_type());
...
void decx::conv::Conv2_single_channel_fp32(decx::_MatrixArray* _src, decx::_Matrix* _kernel, decx::_MatrixArray* _dst, const int flag, de::DH *handle)
{
    if (!decx::cpu::_is_CPU_init()) {
        decx::err::CPU_Not_init<_print>(handle);
        return;
    }
    ...
    case decx::bp::extend_label::_EXTEND_CONSTANT_:
        _dst->re_construct(_src->Type(), _src->Width(), _src->Height(), _src->Array_num(), decx::DATA_STORE_TYPE::Page_Default);
...
_DECX_API_ de::DH 
de::cpu::Filter2D(de::Matrix& src, de::Matrix& kernel, de::Matrix& dst, const int flag, const int _output_type)
{
    de::DH handle;
...
_DECX_API_ void
decx::cpu::Filter2D_Raw_API(decx::_Matrix* _src, decx::_Matrix* _kernel, decx::_Matrix* _dst, const int flag, const int _output_type, de::DH* handle)
{
    if (!decx::cpu::_is_CPU_init()) {
        decx::err::CPU_Not_init<false>(handle);
        return;
    }

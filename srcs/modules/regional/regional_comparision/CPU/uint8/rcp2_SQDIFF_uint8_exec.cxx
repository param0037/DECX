                                        ...
                                        const uint                  _loop)
{
    ...
    if (f_mgr->frag_left_over != 0) {
        ...
        for (int i = 0; i < t1D->total_thread - 1; ++i) {
            t1D->_async_thread[i] = decx::cpu::register_task(&decx::thread_pool, decx::rcp::CPUK::_rcp2_SQDIFF_uint8_ST,
        ...
        t1D->_async_thread[t1D->total_thread - 1] = decx::cpu::register_task(&decx::thread_pool, decx::rcp::CPUK::_rcp2_SQDIFF_uint8_ST,
            tmp_src_ptr, kernel, tmp_dst_ptr,
                                         ...
                                         const uint                  _loop)
{
    ...
    if (f_mgr->frag_left_over != 0) {
        ...
        for (int i = 0; i < t1D->total_thread - 1; ++i) {
            t1D->_async_thread[i] = decx::cpu::register_task(&decx::thread_pool,decx::rcp::CPUK::_rcp2_SQDIFF_norm_uint8_ST,
        ...
        t1D->_async_thread[t1D->total_thread - 1] = decx::cpu::register_task(&decx::thread_pool,decx::rcp::CPUK::_rcp2_SQDIFF_norm_uint8_ST,
            tmp_src_ptr, kernel, tmp_dst_ptr, _k_sq_sum,
            make_uint2(proc_dim.x, f_mgr->frag_left_over), ker_dims, Wsrc, Wdst, reg_WL, _loop);
    }
    else {

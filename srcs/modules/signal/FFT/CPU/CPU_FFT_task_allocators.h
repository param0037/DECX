    ...
    FuncType&& f, const float* __restrict src, double* __restrict dst, const size_t signal_length)
{
    ...
    if (f_mgr->is_left) {
        for (int i = 0; i < t1D->total_thread - 1; ++i) {
            ...
            t1D->_async_thread[i] = register_task(&decx::thread_pool, f, src, dst, signal_length, start_end);
            start_end.x += f_mgr->frag_len;
        }
        start_end.y += f_mgr->frag_left_over;
        t1D->_async_thread[t1D->total_thread - 1] = register_task(&decx::thread_pool, f, src, dst, signal_length, start_end);
    }
    else {
    ...
    const size_t warp_proc_len)
{
    ...
    if (f_mgr->is_left) {
        for (int i = 0; i < t1D->total_thread - 1; ++i) {
            start_end.y += f_mgr->frag_len;
            t1D->_async_thread[i] = register_task(&decx::thread_pool, f, src, dst, signal_length, warp_proc_len, start_end);
            start_end.x += f_mgr->frag_len;
        }
        start_end.y += f_mgr->frag_left_over;
        t1D->_async_thread[t1D->total_thread - 1] = register_task(&decx::thread_pool, f, src, dst, signal_length, warp_proc_len, start_end);
    }
    else {
        for (int i = 0; i < t1D->total_thread; ++i) {
            start_end.y += f_mgr->frag_len;

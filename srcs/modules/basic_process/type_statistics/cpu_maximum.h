...
namespace decx
{
    namespace bp {
        namespace CPUK {
            ...
            */
            _THREAD_FUNCTION_ void
                _maximum_vec8_fp32(const float* src, const size_t len, float* res_vec);
            ...
            */
            _THREAD_FUNCTION_ void
                _maximum_vec4_fp64(const double* src, const size_t len, double* res_vec);
        }
        ...
        * @param res_vec : the result vector in __m256
        */
        static void _maximum_fp32_1D_caller(const float* src, const size_t len, float* res_vec);
        ...
        * @param res_vec : the result vector in __m256
        */
        static void _maximum_fp64_1D_caller(const double* src, const size_t len, double* res_vec);
    }
}
...
_THREAD_FUNCTION_ void
decx::bp::CPUK::_maximum_vec8_fp32(const float* src, const size_t len, float* res_vec)
{
...
_THREAD_FUNCTION_ void
decx::bp::CPUK::_maximum_vec4_fp64(const double* src, const size_t len, double* res_vec)
{
...
static void decx::bp::_maximum_fp32_1D_caller(const float* src, const size_t len, float* res_vec)
{
    ...
    if (fr_mgr.frag_left_over != 0) {
        ...
        for (int i = 0; i < conc_thr - 1; ++i) {
            t1D._async_thread[i] = decx::cpu::register_task(&decx::thread_pool,
                decx::bp::CPUK::_maximum_vec8_fp32, tmp_src, proc_len / 8, res_arr + i);
            tmp_src += proc_len;
        }
        t1D._async_thread[conc_thr - 1] = decx::cpu::register_task(&decx::thread_pool,
            decx::bp::CPUK::_maximum_vec8_fp32, tmp_src, fr_mgr.frag_left_over, res_arr + conc_thr - 1);
    }
    else {
        const size_t proc_len = fr_mgr.frag_len * 8;
...
static void decx::bp::_maximum_fp64_1D_caller(const double* src, const size_t len, double* res_vec)
{
    // the number of available concurrent threads
    const uint conc_thr = decx::cpI.cpu_concurrency;
    decx::utils::frag_manager fr_mgr;
    ...
    if (fr_mgr.frag_left_over != 0) {
        ...
        for (int i = 0; i < conc_thr - 1; ++i) {
            t1D._async_thread[i] = decx::cpu::register_task(&decx::thread_pool,
                decx::bp::CPUK::_maximum_vec4_fp64, tmp_src, proc_len / 4, res_arr + i);
            tmp_src += proc_len;
        }
        t1D._async_thread[conc_thr - 1] = decx::cpu::register_task(&decx::thread_pool,
            decx::bp::CPUK::_maximum_vec4_fp64, tmp_src, fr_mgr.frag_left_over, res_arr + conc_thr - 1);
    }
    else {
        const size_t proc_len = fr_mgr.frag_len * 4;
        for (int i = 0; i < conc_thr; ++i) {
...
namespace de
{
    namespace cpu
    {
        _DECX_API_ de::DH Max_fp32(de::Vector& src, float* res);


        _DECX_API_ de::DH Max_fp32(de::Matrix& src, float* res);


        _DECX_API_ de::DH Max_fp32(de::Tensor& src, float* res);


        _DECX_API_ de::DH Max_fp64(de::Vector& src, double* res);


        _DECX_API_ de::DH Max_fp64(de::Matrix& src, double* res);


        _DECX_API_ de::DH Max_fp64(de::Tensor& src, double* res);
    }
}

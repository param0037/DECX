...
_DECX_API_ de::DH de::cpu::TypeCast(de::Matrix& src, de::Matrix& dst, const int cvt_method)
{
    ...
    if (cvt_method == TypeCast_Method::CVT_FP32_FP64) {
        ...
        decx::type_cast::_cvtf32_ui8_caller2D((float*)_src->Mat.ptr, (int*)_dst->Mat.ptr, make_ulong2(_src->pitch / 8, _src->height),
            _src->pitch, _dst->pitch / 4, cvt_method, &handle);
    }
    else if (cvt_method == CVT_UINT8_FP32) {
        decx::type_cast::_cvtui8_f32_caller2D((float*)_src->Mat.ptr, (float*)_dst->Mat.ptr, make_ulong2(_dst->pitch / 8, _src->height),
            _src->pitch / 4, _dst->pitch);
    }
    else {

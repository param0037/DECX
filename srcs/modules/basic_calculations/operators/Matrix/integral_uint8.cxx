    ...
    const uint pitchsrc, const uint pitchdst)
{
    ...
    for (int i = 0; i < t1D.total_thread - 1; ++i) {
        t1D._async_thread[i] = decx::cpu::register_task(&decx::thread_pool, decx::calc::CPUK::_integral_H_uint8_2D_ST,
    ...
    dst_ptr = (float*)dst;
    for (int i = 0; i < t1D.total_thread - 1; ++i) {
        t1D._async_thread[i] = decx::cpu::register_task(&decx::thread_pool, decx::calc::CPUK::_integral_V_uint8_2D_ST,
    ...
    t1D._async_thread[t1D.total_thread - 1] = decx::cpu::register_task(&decx::thread_pool, decx::calc::CPUK::_integral_V_uint8_2D_ST,
    ...
    const uint pitchsrc, const uint pitchdst)
{
    ...
    dst_ptr = (float*)dst;
    for (int i = 0; i < t1D.total_thread - 1; ++i) {
        t1D._async_thread[i] = decx::cpu::register_task(&decx::thread_pool, decx::calc::CPUK::_integral_V_uint8_f32_2D_ST,
            dst_ptr, make_uint2(f_mgrV.frag_len, proc_dims.y), pitchdst);
        dst_ptr += (size_t)f_mgrV.frag_len << 3;
    }
    frag_proc_dims = make_uint2(f_mgrV.is_left ? f_mgrV.frag_left_over : f_mgrV.frag_len, proc_dims.y);
    t1D._async_thread[t1D.total_thread - 1] = decx::cpu::register_task(&decx::thread_pool, decx::calc::CPUK::_integral_V_uint8_f32_2D_ST,
        dst_ptr, frag_proc_dims, pitchdst);

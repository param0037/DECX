...
#ifndef _CUDA_MUL_gpu_MATRIX_H_
#define _CUDA_MUL_gpu_MATRIX_H_
...
de::DH de::cuda::Mul(de::GPU_Matrix<T>& A, de::GPU_Matrix<T>& B, de::GPU_Matrix<T>& dst)
{
    ...
    const size_t len = _A._element_num;
    decx::dev_Kmul_m(_A.Mat.ptr, _B.Mat.ptr, _dst.Mat.ptr, len);
...
de::DH de::cuda::Mul(de::GPU_Matrix<T>& src, T __x, de::GPU_Matrix<T>& dst)
{
    ...
    const size_t len = _src._element_num;
    decx::dev_Kmul_c(_src.Mat.ptr, __x, _dst.Mat.ptr, len);

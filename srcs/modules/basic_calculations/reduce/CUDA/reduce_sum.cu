...
void decx::reduce::cuda_reduce1D_sum_fp16_fp32_caller_Async(decx::reduce::cuda_reduce1D_configs<de::Half>* _kp_configs, decx::cuda_stream* S)
{
    ...
    _rwpk = _rwpk_arr[0];
    decx::reduce::GPUK::cu_block_reduce_sum1D_fp16_fp32 << <_rwpk._grid_len, _rwpk._block_len,
        0, S->get_raw_stream_ref() >> > ((float4*)_rwpk._src, (float*)_rwpk._dst, _rwpk._proc_len_v, _rwpk._proc_len_v1);
...
void decx::reduce::reduce_sum2D_h_fp32_Async(decx::reduce::cuda_reduce2D_1way_configs<float>* _configs, decx::cuda_stream* S)
{
    ...
    decx::reduce::GPUK::cu_warp_reduce_sum2D_h_fp32_transp << <_rwpk._grid_dims, _rwpk._block_dims,
        0, S->get_raw_stream_ref() >> > ((float4*)_rwpk._src,       (float*)_rwpk._dst, 
                                         _rwpk._calc_pitch_src,     _rwpk._calc_pitch_dst,      _rwpk._calc_proc_dims);
}
...
void decx::reduce::reduce_sum2D_h_fp16_fp32_Async(decx::reduce::cuda_reduce2D_1way_configs<de::Half>* _configs, decx::cuda_stream* S)
{
    using namespace std;
    const std::vector<decx::reduce::RWPK_2D>& _rwpks = _configs->get_rwpks();
    decx::reduce::RWPK_2D _rwpk;
    ...
    if (_rwpks.size() > 1) {
        _rwpk = _rwpks[0];
        decx::reduce::GPUK::cu_warp_reduce_sum2D_h_fp16_fp32 << <_rwpk._grid_dims, _rwpk._block_dims,
            0, S->get_raw_stream_ref() >> > ((float4*)_rwpk._src,       (float*)_rwpk._dst, 
                                             _rwpk._calc_pitch_src,     _rwpk._calc_pitch_dst,      _rwpk._calc_proc_dims);

        for (int i = 1; i < _rwpks.size() - 1; ++i) {
            _rwpk = _rwpks[i];
        ...
        decx::reduce::GPUK::cu_warp_reduce_sum2D_h_fp16_fp32_transp << <_rwpk._grid_dims, _rwpk._block_dims,
            0, S->get_raw_stream_ref() >> > ((float4*)_rwpk._src,       (float*)_rwpk._dst, 
                                             _rwpk._calc_pitch_src,     _rwpk._calc_pitch_dst,      _rwpk._calc_proc_dims);
    }
}
...
void decx::reduce::reduce_sum2D_h_u8_i32_Async(decx::reduce::cuda_reduce2D_1way_configs<uint8_t>* _configs, decx::cuda_stream* S)
{
    ...
    if (_rwpks.size() > 1) {
        ...
        _rwpk = _rwpks[_rwpks.size() - 1];
        decx::reduce::GPUK::cu_warp_reduce_sum2D_h_int32_transp << <_rwpk._grid_dims, _rwpk._block_dims,
            0, S->get_raw_stream_ref() >> > ((int4*)_rwpk._src,       (int32_t*)_rwpk._dst, 
                                             _rwpk._calc_pitch_src,     _rwpk._calc_pitch_dst,      _rwpk._calc_proc_dims);
    }
    else {
        ...
        decx::reduce::GPUK::cu_warp_reduce_sum2D_h_u8_i32_transp << <_rwpk._grid_dims, _rwpk._block_dims,
            0, S->get_raw_stream_ref() >> > ((int4*)_rwpk._src,       (int32_t*)_rwpk._dst, 
                                             _rwpk._calc_pitch_src,     _rwpk._calc_pitch_dst,      _rwpk._calc_proc_dims);
    }
}
...
void decx::reduce::reduce_sum2D_v_fp16_fp32_Async(decx::reduce::cuda_reduce2D_1way_configs<de::Half>* _configs, decx::cuda_stream* S)
{
    const std::vector<decx::reduce::RWPK_2D>& _rwpks = _configs->get_rwpks();
    decx::reduce::RWPK_2D _rwpk;

    _rwpk = _rwpks[0];
    decx::reduce::GPUK::cu_warp_reduce_sum2D_v_fp16_fp32 << <_rwpk._grid_dims, _rwpk._block_dims,
            0, S->get_raw_stream_ref() >> > ((float4*)_rwpk._src,       (float4*)_rwpk._dst, 
                                             _rwpk._calc_pitch_src,     _rwpk._calc_pitch_dst,      _rwpk._calc_proc_dims);
...
void decx::reduce::reduce_sum2D_v_u8_i32_Async(decx::reduce::cuda_reduce2D_1way_configs<uint8_t>* _configs, decx::cuda_stream* S)
{
    ...
    /*std::cout << _rwpk._calc_pitch_src << std::endl;
    std::cout << _rwpk._calc_pitch_dst << std::endl;*/
                                                            ...
                                                            const bool                                  _more_than_flatten)
{
    ...
    decx::reduce::GPUK::cu_warp_reduce_sum2D_flatten_fp16_fp32 << <rwpk_flatten._grid_dims, rwpk_flatten._block_dims,
        0, S->get_raw_stream_ref() >> > ((float4*)src_ptr, (float*)_kp_configs->get_src(), rwpk_flatten._calc_pitch_src, rwpk_flatten._calc_proc_dims);

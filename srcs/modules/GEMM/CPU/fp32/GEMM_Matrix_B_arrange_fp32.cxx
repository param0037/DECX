...
{
    float* local_ptr_src = srcB, * local_ptr_dst = dstB;
    if (f_mgr->is_left) {
        for (int i = 0; i < t2D->total_thread - 1; ++i) {
            t2D->_async_thread[i] = decx::cpu::register_task(&decx::thread_pool, decx::gemm::CPUK::_sort_ST_MatB_fp32,
        ...
        if (is_L8) {
            ...
            t2D->_async_thread[t2D->total_thread - 1] = decx::cpu::register_task(&decx::thread_pool, decx::gemm::CPUK::_sort_ST_MatB_fp32_L8,
                local_ptr_src, local_ptr_dst, _eff_L_len, f_mgr->frag_left_over - 1, WsrcB, WtmpB);
        }
        else {
            t2D->_async_thread[t2D->total_thread - 1] = decx::cpu::register_task(&decx::thread_pool, decx::gemm::CPUK::_sort_ST_MatB_fp32,

...
#ifndef _GEMM_KERNEL_FP32_H_
#define _GEMM_KERNEL_FP32_H_
...
namespace decx
{
    namespace gemm {
        namespace CPUK {
            _THREAD_CALL_
            static void GEMM_AB_fp32_Loop_fixed(const float* __restrict A, const float* __restrict B, float* __restrict dst,
                decx::_C_MM_* __restrict _MMC);


            _THREAD_CALL_
            static void GEMM_AB_fp32_Loop_flex(const float* __restrict A, const float* __restrict B, float* __restrict dst,
                decx::_C_MM_* __restrict _MMC);
            ...
            */
            _THREAD_FUNCTION_
            void GEMM_AB_fp32_AllFixed(const float* A, const float* B, float* dst, const uint pitchA, const uint pitchB, const uint pitchdst,
            ...
            */
            _THREAD_FUNCTION_
            void GEMM_AB_fp32_flexWH(const float* A, const float* B, float* dst, const uint pitchA, const uint pitchB, const uint pitchdst,
                const uint _linear, const uint2 proc_dims);
        }
    }
}
...
static void decx::gemm::CPUK::GEMM_AB_fp32_Loop_fixed(const float* A, const float* B, float* dst, decx::_C_MM_* __restrict _MMC)
{
    ...
    // __linear...
    decx::_GEMM_loopL_fp32_fixed_first(A + dex_A, B + dex_B, dst, _MMC);
...
_THREAD_CALL_
static void decx::gemm::CPUK::GEMM_AB_fp32_Loop_flex(const float* A, const float* B, float* dst, decx::_C_MM_* __restrict _MMC)
{
    size_t dex_B = 0, dex_A = 0;
    const uint _LoopL = _MMC->_linear / 8 / _BLOCKED_GEMM_FP32_LINEAR_;
    const uint _LoopL_left = (_MMC->_linear / 8) % _BLOCKED_GEMM_FP32_LINEAR_;
    // __linear...
    decx::_GEMM_loopL_fp32_flexWH_first(A + dex_A, B + dex_B, dst, _MMC);
    dex_A += _BLOCKED_GEMM_FP32_LINEAR_ * 8;
    dex_B += _BLOCKED_GEMM_FP32_LINEAR_ * 16 * 8;

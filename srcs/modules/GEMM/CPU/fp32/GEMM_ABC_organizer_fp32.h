...
namespace decx
{
    ...
    */
    static void GEMM_ABC_fp32_caller_16x(float* A, float* B, float* C, float* dst, const uint pitchA, const uint pitchB,
    ...
    * @param __linear : _A->width == _B->height, in __m256
    */
    static void GEMM_ABC_fp32_caller_8x(float* A, float* B, float* C, float* dst, float* extra_dst, const uint pitchA, const uint pitchB,
...
static void decx::GEMM_ABC_fp32_caller_16x(float* A,                         float* B, 
                                           float* C,                         float* dst,
                                           ...
                                           const uint linear_len,            decx::utils::_thr_2D* t2D)
{
    ...
    float* A_local_ptr = NULL, * B_local_ptr = NULL, *C_local_ptr = NULL, * dst_local_ptr = NULL;
    ...
    if (f_mgr_H.is_left || f_mgr_W.is_left) {
        for (int i = 0; i < t2D->thread_h - 1; ++i) {
            for (int j = 0; j < t2D->thread_w - 1; ++j) 
            {
                ...
                C_local_ptr = DECX_PTR_SHF_XY<float, float>(C, i * f_mgr_H.frag_len, j * f_mgr_W.frag_len * 16, pitchdst);

                t2D->_async_thread[_L_thr_dex] = decx::cpu::register_task(&decx::thread_pool, decx::gemm::CPUK::GEMM_ABC_fp32_flexWH,
            ...
            C_local_ptr = DECX_PTR_SHF_XY<float, float>(C, i * f_mgr_H.frag_len, (t2D->thread_w - 1) * f_mgr_W.frag_len * 16, pitchdst);
        ...
        for (int j = 0; j < t2D->thread_w - 1; ++j)
        {
            ...
            C_local_ptr = DECX_PTR_SHF_XY<float, float>(C, (t2D->thread_h - 1) * f_mgr_H.frag_len, j * f_mgr_W.frag_len * 16, pitchdst);
        ...
        C_local_ptr = DECX_PTR_SHF_XY<float, float>(C, (t2D->thread_h - 1) * f_mgr_H.frag_len, (t2D->thread_w - 1) * f_mgr_W.frag_len * 16, pitchdst);
        ...
        for (int i = 0; i < t2D->thread_h; ++i) {
            for (int j = 0; j < t2D->thread_w; ++j)
            {
                ...
                t2D->_async_thread[_L_thr_dex] = decx::cpu::register_task(&decx::thread_pool, decx::gemm::CPUK::GEMM_ABC_fp32_AllFixed,
                    A_local_ptr, B_local_ptr, C_local_ptr, dst_local_ptr,
                    pitchA, pitchB, pitchC, pitchdst, linear_len,
                    make_uint2(f_mgr_W.frag_len, f_mgr_H.frag_len));
...
static void decx::GEMM_ABC_fp32_caller_8x(float* A,                         float* B, 
                                          float* C,                         float* dst,
                                          ...
                                          const uint linear_len,            decx::utils::_thr_2D* t2D)
{
    ...
    C_local_ptr = DECX_PTR_SHF_XY<float, float>(C, (t2D->thread_h - 1) * f_mgr_H.frag_len, (t2D->thread_w - 1) * f_mgr_W.frag_len * 16, pitch_ext_dst);

                                         ...
                                         const uint2                   proc_dims)      // _A->width == _B->height, in __m256
{
    ...
    for (int i = 0; i < _LoopHA; ++i) {
        ...
        for (int j = 0; j < _LoopWB; ++j) {
            decx::gemm::CPUK::GEMM_ABC_fp64_Loop_fixed(A + dex_A, B + dex_B, C + dex_C, dst + dex_dst, &_MMC);
                                       ...
                                       const uint2                       proc_dims)     // _A->width == _B->height, in __m256
{
    ...
    for (int i = 0; i < _LoopHA; ++i) {
        ...
        if (_LoopWB_left) {
            ...
            decx::gemm::CPUK::GEMM_ABC_fp64_Loop_flex(A + dex_A, B + dex_B, C + dex_C, dst + dex_dst, &_MMC);

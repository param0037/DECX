    ...
    const int extend_type, void* val)
{
    ...
    _dst->re_construct(_src->type, _src->length + left + right, _dst->_store_type);
    ...
    const uint32_t top, const uint32_t bottom, const int extend_type, void* val)
{
    ...
    _dst->re_construct(_src->Type(), _src->Width() + left + right, _src->Height() + top + bottom, _dst->get_store_type());

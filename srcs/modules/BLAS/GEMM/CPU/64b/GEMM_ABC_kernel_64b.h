...
#include "../fp64/GEMM_ABC_block_kernel_fp64.h"
#include "../cpl32/GEMM_ABC_block_kernel_cpl32.h"
...
template <bool _is_cpl> _THREAD_CALL_
static void decx::gemm::CPUK::GEMM_ABC_fp64_Loop_fixed(const double* A, const double* B, const double* C, double* dst, decx::_C_MM_* __restrict _MMC)
{
    ...
    // __linear...
    if (_is_cpl) { decx::gemm::CPUK::_GEMM_ABC_loopL_cpl32_fixed_first(A + dex_A, B + dex_B, C, dst, _MMC); }
    else{ decx::gemm::CPUK::_GEMM_ABC_loopL_fp64_fixed_first(A + dex_A, B + dex_B, C, dst, _MMC); }
    ...
    if (_LoopL_left) {
        if (_is_cpl) { decx::_GEMM_loopL_cpl32_flexL(A + dex_A, B + dex_B, dst, _LoopL_left, _MMC); }
...
static void decx::gemm::CPUK::GEMM_ABC_fp64_Loop_flex(const double* A, const double* B, const double* C, double* dst, decx::_C_MM_* __restrict _MMC)
{
    ...
    // __linear...
    if (_is_cpl) { decx::gemm::CPUK::_GEMM_ABC_loopL_cpl32_flexWH_first(A + dex_A, B + dex_B, C, dst, _MMC); }
    else{ decx::gemm::CPUK::_GEMM_ABC_loopL_fp64_flexWH_first(A + dex_A, B + dex_B, C, dst, _MMC); }

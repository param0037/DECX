...
void decx::gemm::CPUK::GEMM_AB_fp64_AllFixed(const double* __restrict           A,      
                                             ...
                                             const uint2                        proc_dims)      // _A->width == _B->height, in __m256
{
    ...
    for (int i = 0; i < _LoopHA; ++i) {
        ...
        for (int j = 0; j < _LoopWB; ++j) {
            decx::gemm::CPUK::GEMM_AB_fp64_Loop_fixed<_is_cpl>(A + dex_A, B + dex_B, dst + dex_dst, &_MMC);
...
template <bool _is_cpl> _THREAD_FUNCTION_
void decx::gemm::CPUK::GEMM_AB_fp64_flexWH(const double* __restrict       A,
                                           const double* __restrict       B,      
                                           double* __restrict             dst, 
                                           const uint                    pitchA,      // The pitch of matrix A (in float)
                                           const uint                    pitchB,      // The pitch of TMP matrix B, the same as that of matrix dst (in float)
                                           ...
                                           const uint2                   proc_dims)     // _A->width == _B->height, in __m256
{
    ...
    decx::_C_MM_ _MMC(pitchA, pitchB, pitchdst, _linear, make_uint2(_BLOCKED_GEMM_FP64_WB_, _BLOCKED_GEMM_FP64_HA_));
    ...
    for (int i = 0; i < _LoopHA; ++i) {
        ...
        if (_LoopWB_left) {
            ...
            decx::gemm::CPUK::GEMM_AB_fp64_Loop_flex<_is_cpl>(A + dex_A, B + dex_B, dst + dex_dst, &_MMC);
...
template void decx::gemm::CPUK::GEMM_AB_fp64_AllFixed<true>(const double* A, const double* B, double* dst, const uint pitchA, const uint pitchB, const uint pitchdst,
    const uint _linear, const uint2 proc_dims);

template void decx::gemm::CPUK::GEMM_AB_fp64_AllFixed<false>(const double* A, const double* B, double* dst, const uint pitchA, const uint pitchB, const uint pitchdst,
    const uint _linear, const uint2 proc_dims);


template void decx::gemm::CPUK::GEMM_AB_fp64_flexWH<true>(const double* A, const double* B, double* dst, const uint pitchA, const uint pitchB, const uint pitchdst,
    const uint _linear, const uint2 proc_dims);


template void decx::gemm::CPUK::GEMM_AB_fp64_flexWH<false>(const double* A, const double* B, double* dst, const uint pitchA, const uint pitchB, const uint pitchdst,
    const uint _linear, const uint2 proc_dims);
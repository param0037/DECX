...
#ifndef _GEMM_CPLXF_KERNELS_H_
#define _GEMM_CPLXF_KERNELS_H_
                         ...
                         const double* __restrict    C)
{
    ...
    // !first && ABC: load dst
    if (!_first) {
        _accu = _mm256_castpd_ps(_mm256_load_pd(dst));
    }
    else {
        if constexpr (_ABC) { _accu = _mm256_castpd_ps(_mm256_load_pd(C)); }
        else { _accu = _mm256_castpd_ps(_mm256_setzero_pd()); }
    }
    ...
    for (uint32_t i = 0; i < _linear / 2; ++i) 
    {
        ...
        // 0
        __m256 A_v4 = _mm256_castpd_ps(_mm256_permute_pd(A_palette2, 0b0000));
        __m256 B_v4 = _mm256_castpd_ps(_mm256_load_pd(B_lane + B_dex));
        _accu = decx::dsp::CPUK::_cp4_fma_cp4_fp32(A_v4, B_v4, _accu);
        // 1
        A_v4 = _mm256_castpd_ps(_mm256_permute_pd(A_palette2, 0b1111));
        B_v4 = _mm256_castpd_ps(_mm256_load_pd(B_lane + B_dex + 4));
        _accu = decx::dsp::CPUK::_cp4_fma_cp4_fp32(A_v4, B_v4, _accu);
        B_dex += 8;
    }

    if (_linear % 2) {
        __m256 A_v4 = _mm256_castpd_ps(_mm256_broadcast_sd(A_line + (_linear / 2) * 2));
        __m256 B_v4 = _mm256_castpd_ps(_mm256_load_pd(B_lane + B_dex));
        _accu = decx::dsp::CPUK::_cp4_fma_cp4_fp32(A_v4, B_v4, _accu);
    }
    _mm256_store_pd(dst, _mm256_castps_pd(_accu));
}
                              ...
                              const double* __restrict C)
{
    ...
    __m256 _accu[2];
    if (!_first) {
        _accu[0] = _mm256_castpd_ps(_mm256_load_pd(dst));
        _accu[1] = _mm256_castpd_ps(_mm256_load_pd(dst + 4));
    }
    else {
        if constexpr (_ABC) { 
            _accu[0] = _mm256_castpd_ps(_mm256_load_pd(C));
            _accu[1] = _mm256_castpd_ps(_mm256_load_pd(C + 4));
        }
        else { 
            _accu[0] = _mm256_castpd_ps(_mm256_setzero_pd());
            _accu[1] = _mm256_castpd_ps(_mm256_setzero_pd());
        }
    }
    ...
    for (uint32_t i = 0; i < _linear / 2; ++i) 
    {
        ...
        // 0
        __m256 A_v8 = _mm256_castpd_ps(_mm256_permute_pd(A_palette2, 0b0000));
        __m256 B_v8_0 = _mm256_castpd_ps(_mm256_load_pd(B_lane + B_dex));
        _accu[0] = decx::dsp::CPUK::_cp4_fma_cp4_fp32(A_v8, B_v8_0, _accu[0]);
        __m256 B_v4_1 = _mm256_castpd_ps(_mm256_load_pd(B_lane + B_dex + pitchB_v4 * 4));
        _accu[1] = decx::dsp::CPUK::_cp4_fma_cp4_fp32(A_v8, B_v4_1, _accu[1]);
        // 1
        A_v8 = _mm256_castpd_ps(_mm256_permute_pd(A_palette2, 0b1111));
        B_v8_0 = _mm256_castpd_ps(_mm256_load_pd(B_lane + B_dex + 4));
        _accu[0] = decx::dsp::CPUK::_cp4_fma_cp4_fp32(A_v8, B_v8_0, _accu[0]);
        B_v4_1 = _mm256_castpd_ps(_mm256_load_pd(B_lane + B_dex + 4 + pitchB_v4 * 4));
        _accu[1] = decx::dsp::CPUK::_cp4_fma_cp4_fp32(A_v8, B_v4_1, _accu[1]);
        B_dex += 8;
    }
    ...
    if (_linear & 1) {
        __m256 A_v4 = _mm256_castpd_ps(_mm256_broadcast_sd(A_line + (_linear / 2) * 2));
        __m256 B_v4_0 = _mm256_castpd_ps(_mm256_load_pd(B_lane + B_dex));
        __m256 B_v4_1 = _mm256_castpd_ps(_mm256_load_pd(B_lane + B_dex + pitchB_v4 * 4));
        _accu[0] = decx::dsp::CPUK::_cp4_fma_cp4_fp32(A_v4, B_v4_0, _accu[0]);
        _accu[1] = decx::dsp::CPUK::_cp4_fma_cp4_fp32(A_v4, B_v4_1, _accu[1]);
    }
    _mm256_store_pd(dst, _mm256_castps_pd(_accu[0]));
    _mm256_store_pd(dst + 4, _mm256_castps_pd(_accu[1]));
}
...
decx::blas::CPUK::GEMM_cplxf_dp_kernel(const double* __restrict A_line, 
                                      const double* __restrict B_lane,
                                      double* __restrict dst, 
                                      ...
                                      const double* __restrict C)
{
    ...
    for (uint32_t i = 0; i < _fmgr_L->frag_num - 1; ++i) {
        if constexpr (dual_lane) {
            decx::blas::CPUK::GEMM_cplxf_dp_kernel_frag_dual<_ABC>(A_line + A_dex, B_lane + B_dex, dst, _fmgr_L->frag_len, 
                pitchB_v4, i == 0, C);
        }
        else {
            decx::blas::CPUK::GEMM_cplxf_dp_kernel_frag<_ABC>(A_line + A_dex, B_lane + B_dex, dst, _fmgr_L->frag_len, i == 0,
                C);
        }
    ...
    if constexpr (dual_lane) {
        decx::blas::CPUK::GEMM_cplxf_dp_kernel_frag_dual<_ABC>(A_line + A_dex, B_lane + B_dex, dst, _fmgr_L->last_frag_len,
            pitchB_v4, _fmgr_L->frag_num - 1 == 0, C);
    }
    else {
        decx::blas::CPUK::GEMM_cplxf_dp_kernel_frag<_ABC>(A_line + A_dex, B_lane + B_dex, dst, _fmgr_L->last_frag_len,
                                         ...
                                         const double* __restrict       C)
{
    ...
    for (uint32_t i = 0; i < proc_dims_v4.y; ++i) {
        ...
        for (uint32_t j = 0; j < proc_dims_v4.x / 2; ++j) {
            decx::blas::CPUK::GEMM_cplxf_dp_kernel<true, _ABC>(A + A_dex, B + B_dex, dst + dst_dex, fmgrL, pitchB_v4, C + dst_dex);
            B_dex += pitchB_v4 * 8;
            dst_dex += 8;
        }
        if (proc_dims_v4.x % 2) {
            decx::blas::CPUK::GEMM_cplxf_dp_kernel<false, _ABC>(A + A_dex, B + B_dex, dst + dst_dex, fmgrL, 0, C + dst_dex);
        }
        A_dex += pitchA_v1;
    }
}

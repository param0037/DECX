                                      ...
                                      const double* __restrict          C)
{
    ...
    for (uint32_t i = 0; i < _fmgr_L->frag_num - 1; ++i) {
        if constexpr (dual_lane) {
            decx::blas::CPUK::GEMM_fp64_dp_kernel_frag_dual<_ABC>(A_line + A_dex,   
                                                                  B_lane + B_dex, 
                                                                  dst,              
                                                                  _fmgr_L->frag_len, 
                                                                  i == 0, C);
            ...
            decx::blas::CPUK::GEMM_fp64_dp_kernel_frag<_ABC>(A_line + A_dex,    
                                                             B_lane + B_dex, 
                                                             dst,               
                                                             _fmgr_L->frag_len, 
                                                             i == 0, C);
                                         ...
                                         const double* __restrict    C)
{
    ...
    for (uint32_t i = 0; i < proc_dims_v4.y; ++i) {
        ...
        for (uint32_t j = 0; j < proc_dims_v4.x / 2; ++j) {
            decx::blas::CPUK::GEMM_fp64_dp_kernel<true, _ABC>(A + A_dex, B + B_dex, dst + dst_dex, fmgrL, C + dst_dex);
            B_dex += Llen * 8;
            dst_dex += 8;
        }
        if (proc_dims_v4.x & 1) {
            decx::blas::CPUK::GEMM_fp64_dp_kernel<false, _ABC>(A + A_dex, B + B_dex, dst + dst_dex, fmgrL, C + dst_dex);
        }
        A_dex += pitchA_v1;
    }
}
                                   ...
                                   const double* __restrict                  C)
{
    ...
    for (uint32_t i = 0; i < config->_fmgr_H.frag_num; ++i) 
    {
        ...
        {
            decx::blas::CPUK::GEMM_fp64_block_kernel<_ABC>(A + A_dex,           B + B_dex, 
                                                           dst + dst_dex,       proc_dims, 
                                                           &config->_fmgr_L,    pitchA_v1, 
                                                           Llen,                pitchdst_v1, 
                                                           C + dst_dex);

            B_dex += config->_fmgr_W.frag_len * Llen * 4;

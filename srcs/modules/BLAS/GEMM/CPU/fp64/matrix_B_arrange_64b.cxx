                            ...
                            const uint32_t              Llen)
{
    ...
    for (uint32_t i = 0; i < block_H; ++i) 
    {
        ...
        for (uint32_t j = 0; j < block_W_v8; ++j) {
            _mm256_store_pd(dst + dex_dst, _mm256_load_pd(src + dex_src));
            _mm256_store_pd(dst + dex_dst + 4, _mm256_load_pd(src + dex_src + 4));
                             ...
                             const uint2                    proc_dims_v4)
{
    ...
    for (uint32_t i = 0; i < proc_dims_v4.y; ++i)
    {
        ...
        if (proc_dims_v4.x & 1) {
            _mm256_store_pd(dst + dex_dst, _mm256_load_pd(src + dex_src));
            _mm256_store_pd(dst + dex_dst + 4, _mm256_setzero_pd());
        }
    }
}
    ...
    for (uint32_t i = 0; i < ptime_h; ++i) 
    {
        ...
        if (i < ptime_h - 1 || _LH == 0) 
        {
            for (uint32_t j = 0; j < ptime_w; ++j) {
                decx::blas::CPUK::_matrix_B_arrange_64b_block<block_W_v8, block_H>(src + dex_src,
    ...
    for (uint32_t i = 0; i < t2D->thread_h; ++i) 
    {
        ...
        for (uint32_t j = 0; j < t2D->thread_w - 1; ++j) 
        {
            t2D->_async_thread[i * t2D->thread_w + j] = decx::cpu::register_task_default(
                decx::blas::CPUK::_matrix_B_arrange_64b_exec<2, 16>,

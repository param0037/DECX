...
_THREAD_FUNCTION_
void decx::gemm::CPUK::GEMM_ABC_fp32_AllFixed(const float* __restrict       A,      
                                         ...
                                         const uint                    pitchC,         // The pitch of TMP matrix B, the same as that of matrix dst (in float)
                                         ...
                                         const uint2                   proc_dims)      // _A->width == _B->height, in __m256
{
    ...
    for (int i = 0; i < _LoopHA; ++i) {
        ...
        for (int j = 0; j < _LoopWB; ++j) {
            decx::gemm::CPUK::GEMM_ABC_fp32_Loop_fixed(A + dex_A, B + dex_B, C + dex_C, dst + dex_dst, &_MMC);
            dex_dst += _BLOCKED_GEMM_FP32_WB_ * 16;
            dex_C += _BLOCKED_GEMM_FP32_WB_ * 16;
        }
        dex_dst += ((size_t)pitchdst * _BLOCKED_GEMM_FP32_HA_ - (16 * proc_dims.x));
        dex_C += ((size_t)pitchC * _BLOCKED_GEMM_FP32_HA_ - (16 * proc_dims.x));
...
_THREAD_FUNCTION_
void decx::gemm::CPUK::GEMM_ABC_fp32_flexWH(const float* __restrict           A,
                                       const float* __restrict           B,
                                       const float* __restrict           C,
                                       float* __restrict                 dst, 
                                       const uint                        pitchA,      // The pitch of matrix A (in float)
                                       const uint                        pitchB,      // The pitch of TMP matrix B, the same as that of matrix dst (in float)
                                       const uint                        pitchC,      // The pitch of TMP matrix C, the same as that of matrix dst (in float)
                                       const uint                        pitchdst,    // The pitch of matrix dst (in float)
                                       const uint                        _linear,
                                       const uint2                       proc_dims)     // _A->width == _B->height, in __m256
{
    ...
    decx::_C_MM_ _MMC(pitchA, pitchB, pitchdst, _linear, make_uint2(_BLOCKED_GEMM_FP32_WB_, _BLOCKED_GEMM_FP32_HA_), pitchC);
    ...
    for (int i = 0; i < _LoopHA; ++i) {
        ...
        dex_C += ((size_t)pitchC * _BLOCKED_GEMM_FP32_HA_ - (16 * _BLOCKED_GEMM_FP32_WB_ * _LoopWB));
        dex_A += (size_t)pitchA * _BLOCKED_GEMM_FP32_HA_;

        if (_LoopWB_left) {
            _MMC._proc_dims.x = proc_dims.x % _BLOCKED_GEMM_FP32_WB_;
            decx::gemm::CPUK::GEMM_ABC_fp32_Loop_flex(A + dex_A, B + dex_B, C + dex_C, dst + dex_dst, &_MMC);
            dex_dst += 16 * _LoopWB_left;
            dex_C += 16 * _LoopWB_left;
        }
    }
    if (_LoopHA_left) {
        dex_B = 0;
        ...
        dex_C += ((size_t)pitchC * _LoopHA_left - (16 * _BLOCKED_GEMM_FP32_WB_ * _LoopWB));

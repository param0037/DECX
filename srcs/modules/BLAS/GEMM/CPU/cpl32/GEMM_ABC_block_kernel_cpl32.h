...
#ifndef _GEMM_ABC_BLOCK_KERNEL_CPL32_H_
#define _GEMM_ABC_BLOCK_KERNEL_CPL32_H_
...
#include "GEMM_block_kernel_cpl32.h"
...
namespace decx
{
    namespace gemm {
        namespace CPUK {
            ...
            */
            _THREAD_CALL_
            static void _GEMM_ABC_loopL_cpl32_fixed_first(const double* A, const double* B, const double *C, double* dst, 
                const decx::_C_MM_* _MMC_props);



            _THREAD_CALL_
            static void _GEMM_ABC_loopL_cpl32_flexWH_first(const double* A, const double* B, const double *C, double* dst, 
                const decx::_C_MM_* _MMC_props);
            ...
            */
            _THREAD_CALL_
            static void _GEMM_ABC_loopL_cpl32_flexL_first(const double* A, const double* B, const double *C, double* dst, 
                const decx::_C_MM_* _MMC_props);
        }
    }
}
...
_THREAD_CALL_
static void decx::gemm::CPUK::_GEMM_ABC_loopL_cpl32_fixed_first(const double* __restrict    A, 
                                                   ...
                                                   const decx::_C_MM_*    _MMC_props)
{
    ...
    for (int i = 0; i < _BLOCKED_GEMM_FP64_HA_; ++i) {
        ...
        for (int j = 0; j < _BLOCKED_GEMM_FP64_WB_; ++j) {
            tmp_dex_A = dex_A;
            _accu0._vd = _mm256_load_pd(C + dex_C);
            _accu1._vd = _mm256_load_pd(C + dex_C + 4);
...
_THREAD_CALL_
static void decx::gemm::CPUK::_GEMM_ABC_loopL_cpl32_flexWH_first(const double* __restrict    A, 
                                                    const double* __restrict    B,
                                                    const double* __restrict    C,
                                                    double* __restrict          dst,
                                                    const decx::_C_MM_*         _MMC_props)
{
    ...
    for (int i = 0; i < _MMC_props->_proc_dims.y; ++i) {
        ...
        for (int j = 0; j < _MMC_props->_proc_dims.x; ++j) {
            tmp_dex_A = dex_A;
            _accu0._vd = _mm256_load_pd(C + dex_dst);
            _accu1._vd = _mm256_load_pd(C + dex_dst + 4);
#pragma unroll _BLOCKED_GEMM_FP64_LINEAR_
            for (int k = 0; k < _BLOCKED_GEMM_FP64_LINEAR_; ++k) {
                reg_A._vd = _mm256_load_pd(A + tmp_dex_A);
...
_THREAD_CALL_
static void decx::gemm::CPUK::_GEMM_ABC_loopL_cpl32_flexL_first(const double* __restrict    A,
                                                   const double* __restrict    B, 
                                                   const double* __restrict    C, 
                                                   double* __restrict          dst, 
                                                   const decx::_C_MM_*         _MMC_props)
